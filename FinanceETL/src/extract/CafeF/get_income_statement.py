...
class IncomeStatementCrawler(BaseCrawler):
    ...
    def get_multi_table_quarter(self, company_name=None, year=None, number_quarter=4, quarter=1):
        ...
        column_names = self.generate_column_quarter_names(number_quarter=number_quarter, year=year, quarter=quarter, name_of_tables=f"Income_Statement_Quarter") 
    ...
    def get_multi_table_year(self, company_name=None, year=None, number_year=CONST_TABLE):
        ...
        final_table = None
        column_names = self.generate_column_year_names(number_year=number_year, year=year, name_of_tables=f"Income_Statement_Annual") 
        for _ in range(round_merge):
            
            __table = self.extract_table_content(generate_income_statement_url(company_name, str(year), "1 YEAR", str(0)), 4)
            __table = __table.iloc[:, :-1]
            if final_table is None:
                final_table = __table
            else:
