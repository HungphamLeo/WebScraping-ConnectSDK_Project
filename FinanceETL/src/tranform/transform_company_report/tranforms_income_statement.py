...
class TransformIncomeStatement(TransformStatementBase):
    ...
    def transform_income_statement_data(self, input_table, source: str):
        ...
        for year, data in data_dict.items():
            if source == Config.ALPHA_VANTAGE:
                transformed_dict[year] = self.mapping_alpha_vantage_income_statement(data)
            elif source == Config.CAFE_F:
                transformed_dict[year] = self.mapping_cafeF_income_statement(data)
            else:
