...
class TransformBalanceSheet(TransformStatementBase):
    ...
    def transform_balance_sheet_data(self, input_table, source: str):
        ...
        for year, data in data_dict.items():
            if source == Config.ALPHA_VANTAGE:
                transformed_dict[year] = self.mapping_alpha_vantage_balance_sheet(data)
            elif source == Config.CAFE_F:
                transformed_dict[year] = self.mapping_cafeF_balance_sheet(data)
            else:
